...
class Service3Payment {
  ...
  runClass() {
    ...
    async function initializeCard(payments) {
      ...
      await card.attach('#booking-modal-s3-card-container');
    ...
    async function createPayment(token) {
      const bodyData = {
        ...
        service_name: 'Posh Session' // _REPLACE_
      };
    ...
    function displayPaymentResults(status) {
      ...
      const statusContainer = document.getElementById(
        'booking-modal-s3-payment-status-container'
      );
      if (status === 'SUCCESS') {
    ...
    document.addEventListener('DOMContentLoaded', async function () {
      ...
      const cardButton = document.getElementById('booking-modal-s3-card-button');
...
const s3PaymentComponent = new Service3Payment()

module.exports = { s3PaymentComponent }