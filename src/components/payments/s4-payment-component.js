...
const service4Comp = require('../book-a-service/service-4.js')

class Service4Payment {
  constructor() {
  ...
  runClass() {
    ...
    async function initializeCard(payments) {
      ...
      await card.attach('#booking-modal-s4-card-container');
    ...
    async function createPayment(token) {
      const bodyData = {
        locationId,
        sourceId: token,
        amount: JSON.parse(localStorage.getItem('service4Price')),
        ...
        service_name: 'Unique Session' // _REPLACE_
      };
    ...
    function displayPaymentResults(status) {
      ...
      const statusContainer = document.getElementById(
        'booking-modal-s4-payment-status-container'
      );
      if (status === 'SUCCESS') {
    ...
    document.addEventListener('DOMContentLoaded', async function () {
      ...
      const cardButton = document.getElementById('booking-modal-s4-card-button');
...
const s4PaymentComponent = new Service4Payment()

module.exports = { s4PaymentComponent }